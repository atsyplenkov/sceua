...
# Bicycle infrastructure transfer using ANIME interpolation (extensive variable)
# Flows are extensive, so we use interpolate_extensive()
bicycle_transfer <- tibble::tibble(
  target_id = target_sample_1$target_id,
  go_dutch  = anime::interpolate_extensive(
    x = npt_sf$all_fastest_bicycle_go_dutch,
    matches = match_matrix_1
  )
)
