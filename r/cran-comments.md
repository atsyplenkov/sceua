...
Rust dependencies are vendored via `rextendr::vendor_crates()` and built offline with `cargo build --offline`. No Rust crates are downloaded during installation. The source tarball is approximately 0.55 MB. Development-only Rust dependencies and benchmarks are stripped from the bootstrapped CRAN source before vendoring and remain available in the repository's Rust crate.
